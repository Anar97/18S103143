from src.Go.game.game import Game
from src.Go.board.position import Position
from src.Go.player.go_player import GoPlayer

"""
Inherited from Game (& relevant):
* properties:
    - board
    - running
    - current_player
...
class GoGame(Game):
    ...
    def __init__(self, size=DEFAULT_GOBAN_SIZE, tiemu=DEFAULT_TIEMU, handicap=0):
    ...
    def _end_game(self):
        ...
        if self.black_player.score < self.white_player.score + self.tiemu:
            self.winner = self.white_player

        elif self.black_player.score > self.white_player.score + self.tiemu:
