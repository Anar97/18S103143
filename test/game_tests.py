...
import src.Go.game.game as game
from src.Go.board.position import Position
from src.Go.player.player import Player
import unittest


class GameBasicFunctionalityTests(unittest.TestCase):
    def setUp(self):
        x = Player("X")
        o = Player("O")
        self.players = [x, o]
    ...
    def test_game_stops_if_board_is_full(self):
        ...
        game = game.Game(self.players, size_of_board_with_one_field)
        p = Position(0, 0)
        game.make_move(p)
    ...
    def test_board_remains_empty_if_move_is_invalid(self):
        size_of_board_with_one_field = 1
        g = game.Game(self.players, size_of_board_with_one_field)
        p = Position(5, 5)
        g.make_move(p)
    ...
    def test_game_changes_turns_automatically_when_players_make_valid_moves(self):
        x, y, o, z = Player("X"), Player("Y"),\
            Player("O"), Player("Z")

        players = [x, y, o, z]
        g = game.Game(players, len(players))
